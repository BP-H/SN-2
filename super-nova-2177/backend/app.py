...
def _rate_limit_identity(request: Request) -> str:
    ...
    if authorization.lower().startswith("bearer ") and jwt is not None:
...
def _rate_limit_attempt(request: Request) -> tuple[Optional[str], Optional[int]]:
...
async def commons_rate_limit_middleware(request: Request, call_next):
    bucket, retry_after = _rate_limit_attempt(request)
